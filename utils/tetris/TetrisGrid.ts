...
export class TetrisGrid {
  ...
  constructor() {
    this.grid = Array.from({ length: TetrisGameConstants.GRID_HEIGHT }, () =>
  ...
  setValue(
    ...
    if (x < 0 || y < 0) return;
    if (
      x > TetrisGameConstants.GRID_WIDTH ||
      y > TetrisGameConstants.GRID_HEIGHT
    )
      return;
  ...
  verifyCompleteLines() {
    ...
    for (let j = TetrisGameConstants.GRID_HEIGHT - 1; j >= 0; --j) {
      let full = this.grid[j].every((element) => {
